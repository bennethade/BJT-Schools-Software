      ...
      <div class="container-fluid">
         <div class="card">
            <div class="card-header">
               <h3 class="card-title">Search PTC</h3>
            </div>
                  ...
                  <div class="row">
                    ...
                    <div class="form-group col-md-3">
                     ...
                     <div class="form-group col-md-3">
                        ...
                        <a href="{{ route('ptc.view') }}" class="btn btn-success" style="margin-top: 32px;">Refresh</a>
                     </div>
                  </div>
               </div>
            </form>
         ...
         <div class="row">
            ...
            <div class="col-md-12">
               ...
               @if (!empty($getSubject) && !empty($getSubject->count()))
                  ...
                  <div class="card">
                     <div class="card-header">
                        <h3 class="card-title">PTC Entry</h3>
                     </div>
                     <!-- /.card-header -->
                                          ...
                                          <tr>
                                             ...
                                             @foreach ($getSubject as $subject)
                                                @php
                                                   ...
                                                   $getPTC = $subject->getPTC(Request::get('class_id'), Request::get('exam_id'), $student->id, $subject->subject_id);
                                                ...
                                                <td>
                                                   ...
                                                   <div style="margin-bottom: 15px;">
                                                      Comment
                                                      <textarea name="mark[{{ $i }}][comment]" id="comment_{{ $student->id }}{{ $subject->subject_id }}" cols="" rows="6" placeholder="Subject Comment" class="form-control" style="width: 250px;">{{ !empty($getPTC->comment) ? $getPTC->comment : '' }}</textarea>
                                                   </div>
                                             ...
                                             <td style="min-width: 250px;">
                                                ...
                                                @if (Auth::user()->user_type == 'Super Admin' || Auth::user()->user_type == 'School Admin' || Auth::user()->user_type == '1')
                                                    <a href="{{ route('ptc.view_single',[Request::get('class_id'), Request::get('exam_id'), $student->id]) }}" class="btn btn-sm btn-warning" style="margin-top:30px;">View PTC</a>
                                                    {{-- <a href="{{ url('admin/ptc/view_single?exam_id=' . Request::get('exam_id') . '&student_id=' . $student->id) }}" class="btn btn-sm btn-warning" style="margin-top:30px;" target="_blank">View PTC</a> --}}
                                                @else
                                                    <a href="{{ url('other_roles/ptc/view_single?class_id=' . Request::get('class_id') . '&exam_id=' . Request::get('exam_id') . '&student_id=' . $student->id) }}" class="btn btn-sm btn-warning" style="margin-top:30px;" target="_blank">View PTC</a>
                                                @endif
...
////FOR MULTIPLE SAVE BUTTON
   $(document).ready(function() {
        ...
        $(document).on('click', '.SaveSingleSubject', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('admin/ptc/single_submit_ptc') }}",
                data: {
                "_token": "{{ csrf_token() }}",
                // id: id,
                student_id: student_id,
