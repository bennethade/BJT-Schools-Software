                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <a href="{{ route('examinations.nursery_midterm.goals') }}" class="btn btn-success" style="margin-top: 32px;">Refresh</a>
                            </div>
                        </div>
                    </div>
                </form>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            @if (Auth::user()->user_type == 'Super Admin' || Auth::user()->user_type == 'School Admin' || Auth::user()->user_type == '1')
                                <a href="{{ url('admin/examinations/print_nursery_midterm_goals?class_id=' . $getSingleClassName->class_id . '&exam_id=' . Request::get('exam_id') . '&student_id=' . Request::get('student_id')) }}" class="btn btn-sm btn-warning float-right" style="" target="_blank">Print Result</a>
                            @else
                                <a href="{{ url('other_roles/print_nursery_midterm_goals/print?class_id=' . $getSingleClassName->class_id . '&exam_id=' . Request::get('exam_id') . '&student_id=' . Request::get('student_id')) }}" class="btn btn-sm btn-warning float-right" style="" target="_blank">Print Result</a>
                            @endif
...
$(document).on('click', '.SaveSubject', function() {
    ...
    if (class_id && exam_id && student_id && subject_id && category_id) {
        $.ajax({
            type: 'POST',
            url: '{{ route("examinations.save_midterm.goal") }}', // Create a route for saving goals
            data: {
                "_token": "{{ csrf_token() }}",
                class_id: class_id,
                exam_id: exam_id,
