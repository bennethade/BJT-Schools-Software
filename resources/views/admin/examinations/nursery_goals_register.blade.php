...
<div class="content-wrapper">
   <section class="content-header">
      <div class="container-fluid">
         <div class="row mb-2">
            <div class="col-sm-12">
               <h1>Nursery Exam Subject Goals <span style="color: red; font-size:18px;">(For Exam ONLY)</span></h1>
            </div>
         </div>
      </div>
   </section>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select name="student_id" id="getStudent" class="form-control" required>
                                    ...
                                    @if(isset($getStudentList))  <!-- If there is a list of students loaded -->
                                        @foreach ($getStudentList as $student) <!-- Display all students in the dropdown -->
                                            <option value="{{ $student->student_id }}" 
                                                {{ (Request::get('student_id') == $student->student_id) ? 'selected' : '' }}>
                                                {{ $student->user_last_name }} {{ $student->user_name }} {{ $student->user_other_name }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <a href="{{ route('examinations.nursery_goals') }}" class="btn btn-success" style="margin-top: 32px;">Refresh</a>
                            </div>
                        </div>
                    </div>
                </form>
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            @if (Auth::user()->user_type == 'Super Admin' || Auth::user()->user_type == 'School Admin' || Auth::user()->user_type == '1')
                                <a href="{{ url('admin/examinations/print_nursery_goals?class_id=' . $getSingleClassName->class_id . '&exam_id=' . Request::get('exam_id') . '&student_id=' . Request::get('student_id')) }}" class="btn btn-sm btn-warning float-right" style="" target="_blank">Print Result</a>
                            @else
                                <a href="{{ url('other_roles/print_nursery_goals/print?class_id=' . $getSingleClassName->class_id . '&exam_id=' . Request::get('exam_id') . '&student_id=' . Request::get('student_id')) }}" class="btn btn-sm btn-warning float-right" style="" target="_blank">Print Result</a>
                            @endif
   ...
   $(function () {
    $('#getClass, #getExam').change(function() {
        ...
        if (class_id && exam_id) {
            $.ajax({
                type: "POST",
                url: "{{ url('admin/ajax_get_student') }}",
                data: {
                    "_token": "{{ csrf_token() }}",
                    class_id: class_id,
                    exam_id: exam_id,
...
$(document).on('click', '.SaveSubject', function() {
    ...
    if (class_id && exam_id && student_id && subject_id && category_id) {
        $.ajax({
            type: 'POST',
            url: '{{ route("examinations.save_subject_goal") }}', // Create a route for saving goals
            data: {
                "_token": "{{ csrf_token() }}",
                class_id: class_id,
                exam_id: exam_id,
