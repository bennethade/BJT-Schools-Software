...
<div class="content-wrapper">
   <section class="content-header">
      <div class="container-fluid">
         <div class="row mb-2">
            <div class="col-sm-6">
               <h1>Nursery Exam Subject Goals <span style="color: red; font-size:18px;">(For Exam ONLY)</h1>
            </div>
         </div>
      </div>
   </section>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-3">
                                ...
                                <select name="class_id" id="getClass" class="form-control" required>
                                    ...
                                    @foreach ($getClass as $class)
                                        <option {{ (Request::get('class_id') == $class->id) ? 'selected' : '' }} value="{{ $class->id }}">{{ $class->class_name }} {{ $class->class_description }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-md-3">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <a href="{{ route('teacher.goals.register') }}" class="btn btn-success" style="margin-top: 32px;">Refresh</a>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                @if(!empty(Request::get('class_id')) && !empty(Request::get('exam_id')) && !empty(Request::get('student_id')))
                    <div class="card">
                        <div class="card-header">
                            ...
                            <a href="{{ url('teacher/examinations/print_nursery_goals?class_id=' . $getSingleClassName->class_id . '&exam_id=' . Request::get('exam_id') . '&student_id=' . Request::get('student_id')) }}" class="btn btn-sm btn-warning float-right" style="" target="_blank">Print Result</a>
                        </div>
...
$(document).on('click', '.SaveSubject', function() {
    ...
    if (class_id && exam_id && student_id && subject_id && category_id) {
        $.ajax({
            type: 'POST',
            url: '{{ route("teacher.goals.save_subject_goal") }}', // Create a route for saving goals
            data: {
                "_token": "{{ csrf_token() }}",
                class_id: class_id,
                exam_id: exam_id,
