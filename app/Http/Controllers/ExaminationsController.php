...
class ExaminationsController extends Controller
{
    ...
    public function singleSubmitMarksRegister(Request $request)
    {
        ...
        if($full_mark >= $total_mark)
        {
            ...
            {
                if(!empty($ca) || !empty($project) || !empty($exam))
                {
                    $save               = new MarksRegister;
                    $save->created_by   = Auth::user()->id;
                }
    ...
    public function behaviorChartSubmit(Request $request)
    {
        ...
        ///### FOR COBENA
        $behavior->number_of_times_present          = $request->number_of_times_present;
        $behavior->number_of_times_absent           = $request->number_of_times_absent;


        $behavior->creative_activities              = $request->creative_activities;
        $behavior->handling_tools                   = $request->handling_tools;
        $behavior->handwriting                      = $request->handwriting;
        $behavior->physical_activities              = $request->physical_activities;
        $behavior->attitudes                        = $request->attitudes;
        $behavior->value_system                     = $request->value_system;
        $behavior->interest_and_appreciation        = $request->interest_and_appreciation;
        $behavior->inter_personal_relationship	    = $request->inter_personal_relationship	;
        $behavior->emotional_adjustment             = $request->emotional_adjustment;
        
        $behavior->class_tutor_comment              = $request->class_tutor_comment;
        $behavior->head_teacher_remark              = $request->head_teacher_remark;
    ...
    {
        $term1_Id = $request->exam_id - 2;
        $term2_Id = $request->exam_id - 1;
        ...
        $assignedSubjects = StudentSubject::where('student_id', $request->student_id)
            ->pluck('subject_id')
            ->unique()
            ->toArray();
        ...
        foreach ($subjects as $subject) {
            $subjectResult = [
                ...
                '1st_ca' => 0, '1st_project' => 0, '1st_exam' => 0,
                '2nd_ca' => 0, '2nd_project' => 0, '2nd_exam' => 0,
                '3rd_ca' => 0, '3rd_project' => 0, '3rd_exam' => 0,
                'overall_total' => 0,
                'weighted_average' => 0,
            ];

            foreach ($termMap as $termId => $label) {
                $mark = MarksRegister::where('student_id', $request->student_id)
                    ->where('subject_id', $subject->id)
                    ->where('exam_id', $termId)
                    ->first();

                if ($mark) {
                    $subjectResult["{$label}_ca"] = $mark->ca;
                    $subjectResult["{$label}_project"] = $mark->project;
                    $subjectResult["{$label}_exam"] = $mark->exam;

                    $subjectResult['overall_total'] += $mark->ca + $mark->project + $mark->exam;
                }
            }

            $firstTotal = $subjectResult['1st_ca'] + $subjectResult['1st_project'] + $subjectResult['1st_exam'];
            $secondTotal = $subjectResult['2nd_ca'] + $subjectResult['2nd_project'] + $subjectResult['2nd_exam'];
            $thirdTotal = $subjectResult['3rd_ca'] + $subjectResult['3rd_project'] + $subjectResult['3rd_exam'];

            $termCount = 0;
            if ($firstTotal > 0) $termCount++;
            if ($secondTotal > 0) $termCount++;
            if ($thirdTotal > 0) $termCount++;
        ...
        $data['grandTotal'] = array_sum(array_column($results, 'overall_total'));
        $data['totalSubjects'] = count($results);
        $data['overallAverage'] = $data['totalSubjects'] > 0 ? round(array_sum(array_column($results, 'weighted_average')) / $data['totalSubjects'], 2) : 0;

        $data['subjectsPassed'] = collect($results)->where('weighted_average', '>=', 50)->count();
        $data['subjectsFailed'] = collect($results)->where('weighted_average', '<', 50)->count();

        if ($data['overallAverage'] >= 96) {
            $data['generalRating'] = 'Exceptional Performance';
        } elseif ($data['overallAverage'] >= 91) {
            $data['generalRating'] = 'Working at expected level';
        } elseif ($data['overallAverage'] >= 80) {
            $data['generalRating'] = 'Skilled performance working towards Meeting expected level';
        } elseif ($data['overallAverage'] >= 70) {
            $data['generalRating'] = 'Good Performance';
        } elseif ($data['overallAverage'] >= 60) {
            $data['generalRating'] = 'Satisfactory performance';
        } elseif ($data['overallAverage'] >= 50) {
            $data['generalRating'] = 'Working below expected level';
        } else {
            $data['generalRating'] = 'Unacceptable Performance';
        }
